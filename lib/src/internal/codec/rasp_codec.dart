...
class RaspCodec {
  static RASPResult resultFromKey(String key) => switch (key) {
        ...
        'UserCADetected' => RASPResult.userCADetected,
        _ => RASPResult.secure,
      };
  ...
  static RASPExtendedResult extendedResultFromMap(Map<dynamic, dynamic> data) {
    return RASPExtendedResult(
      checkType: RASPCheckType.fromKey(data['checkType'] as String),
      vulnerable: data['vulnerable'] as bool,
      threatAction: actionFromKey(data['threatAction'] as String),
    );
  }

  static Map<String, dynamic> configToMap(RASPConfig config) {
    return {
      if (config.trustedStores != null) 'trustedStores': config.trustedStores,
      if (config.threatActionConfig != null)
        'threatActionConfig': _actionConfigToMap(config.threatActionConfig!),
    };
  }
  ...
  static Map<String, String> _actionConfigToMap(ThreatActionConfig c) => {
        ...
        'deviceSecurityStateUnsafeDetectedAction':
            actionToKey(c.deviceSecurityStateUnsafeDetectedAction),
        'userCADetectedAction': actionToKey(c.userCADetectedAction),
      };
}
