...
class RASPChecker {
  ...
  RASPChecker.builder({
    ...
    this.rootDetector = true,
    this.tampering = true,
    this.emulator = true,
    this.deviceSecurityState = true,
    this.userCA = true,
  });
  ...
  Future<String> _ensureBuilt() async {
    _assertNotDisposed();
    return _checkerId ??=
        await CShieldSdkPlatform.instance.raspBuild(flags: {
  ...
  Future<void> setRASPConfig(RASPConfig config) async {
    ...
    await CShieldSdkPlatform.instance.raspSetConfig(
      checkerId: id,
      config: RaspCodec.configToMap(config),
    );
  }

  Future<RASPResult> quickCheck() async {
    final id = await _ensureBuilt();
    final key = await CShieldSdkPlatform.instance.raspQuickCheck(
      checkerId: id,
    );
  ...
  Stream<RASPExtendedResult> subscribe({
    ...
    try {
      await CShieldSdkPlatform.instance.raspSubscribe(
        checkerId: id,
      ...
      await CShieldSdkPlatform.instance
          .raspCancelSubscribe(subscriptionId: subscriptionId)
          .catchError((_) {});
    }
  }
  ...
  Future<void> dispose() async {
    ...
    if (_checkerId != null) {
      await CShieldSdkPlatform.instance
          .raspDispose(checkerId: _checkerId!)
          .catchError((_) {});
    }
  }
